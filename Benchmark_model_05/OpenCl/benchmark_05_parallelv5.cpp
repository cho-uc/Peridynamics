...
Serialized all kernels that are slower in GPU
	...
	-u_reinitialization 
Set critical time step as parallel
Serial result : 9340 ms (10,000 nodes) 
Parallel result : 7223 ms
Parallel result with partial serialized: 6688 ms

This perform worse than v.2
*/
...
int main(int argc, char **argv){
	...
	cl::Buffer buffer_k_bulk_mod(context, CL_MEM_READ_ONLY, sizeof(float));
	...
	queue.enqueueWriteBuffer(buffer_k_bulk_mod, CL_TRUE, 0, sizeof(float), &k_bulk_mod);
	...
	vector<float>V_dot_C_array(node,0);
	cl::Buffer buffer_V_dot_C_array(context, CL_MEM_READ_WRITE, sizeof(float)*node);
		
	cl::Kernel cl_kernel_critical_time_step(clProgram, "critical_time_step");

	cl_kernel_critical_time_step.setArg(0, buffer_neighbor_list_pointer);
	cl_kernel_critical_time_step.setArg(1, buffer_neighbor_list);
	cl_kernel_critical_time_step.setArg(2, buffer_delta_V);
	cl_kernel_critical_time_step.setArg(3, buffer_x);
	cl_kernel_critical_time_step.setArg(4, buffer_y);
	cl_kernel_critical_time_step.setArg(5, buffer_z);
	cl_kernel_critical_time_step.setArg(6, buffer_V_dot_C_array);
	cl_kernel_critical_time_step.setArg(7, buffer_iter_neighbor_list);
	cl_kernel_critical_time_step.setArg(8, buffer_node);
	cl_kernel_critical_time_step.setArg(9, buffer_small_delta);
	cl_kernel_critical_time_step.setArg(10, buffer_k_bulk_mod);
	
	queue.enqueueNDRangeKernel(cl_kernel_critical_time_step, cl::NullRange, globalRange, cl::NullRange, NULL);
		
	queue.enqueueReadBuffer(buffer_V_dot_C_array, CL_TRUE, 0, sizeof(float) * node, &V_dot_C_array[0]);
