...
Serialized all kernels that are slower in GPU
	...
	-u_reinitialization 
Replace calloc with 'new' for array initialization
...
int main(int argc, char **argv){
	...
	float *u_x_n0 = new float[node];
	float *u_y_n0 = new float[node];
	float *u_z_n0 = new float[node];
	
	float *u_x_n1 = new float[node];
	float *u_y_n1 = new float[node];
	float *u_z_n1 = new float[node];
	
	float *u_dot_x_n0 = new float[node];
	float *u_dot_y_n0 = new float[node];
	float *u_dot_z_n0 = new float[node];
	
	float *u_dot_x_nhalf = new float[node];
	float *u_dot_y_nhalf = new float[node];
	float *u_dot_z_nhalf = new float[node];
	
	float *u_doubledot_x_n0 = new float[node];
	float *u_doubledot_y_n0 = new float[node];
	float *u_doubledot_z_n0 = new float[node];
	...
	/*discretize_blocks<<<gridSize_02, Blocksize_02>>>(buffer_delta_x, buffer_delta_y, buffer_delta_z,
	...
	*/
	...
	/*weighted_vol<<<gridSize_03, blockSize_03>>>(buffer_neighbor_list_pointer,
	...
	*/
	...
	for (size_t t_step = 0; t_step < num_steps; ++t_step){
		...
		/*cal_dilatation<<<gridSize, blockSize>>>( buffer_neighbor_list_pointer,
	...
	delete [] delta_V;
	delete [] u_x_n0;	delete [] u_y_n0; 	delete [] u_z_n0;
	delete [] u_x_n1;	delete [] u_y_n1; 	delete [] u_z_n1;
	delete [] u_dot_x_n0;	delete [] u_dot_y_n0;	delete [] u_dot_z_n0;	
	delete [] u_dot_x_n0; delete [] u_dot_y_n0; delete [] u_dot_z_n0;
	delete [] u_dot_x_nhalf; delete [] u_dot_y_nhalf; delete [] u_dot_z_nhalf;
	delete [] u_doubledot_x_n0; delete [] u_doubledot_y_n0; delete [] u_doubledot_z_n0;
