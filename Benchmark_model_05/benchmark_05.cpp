...
int main(int argc, char **argv){
	...
	float delta_t=0.0;
	if (delta_t_critical>1.0e-7){
		delta_t=1.0e-7;
		cout<<"Use template value "<<delta_t<<" for delta_t"<<endl;
		}
	else{
		delta_t=delta_t_critical;
	...
	/*
	for (size_t i = 0; i < node; ++i) {
		b_x[i] = load/(node*width*heigth*length); //load to all points
	}
	*/
	...
	for (size_t t_step = 0; t_step < num_steps; ++t_step){
		...
		for (size_t i = 0; i < node; ++i) {
			...
			//Apply BC
			for (size_t j = 0; j <(ndivy*ndivz); ++j) { // in the beginning of the block
